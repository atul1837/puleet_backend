...
def allot_branches_based_on_preferences_and_rank():
    ...
    for candidate in candidates:
        ...
        for preference in preferences:
            ...
            if branch.general_seats > Allotment.general_count(branch):
                Allotment.objects.create(user=candidate.user, branch=branch, category=Allotment.GENERAl, preference=preference)
                break
            elif candidate.category == Profile.SCHEDULED_CASTE and branch.sc_seats > Allotment.sc_count(branch):
                Allotment.objects.create(user=candidate.user, branch=branch, category=Allotment.SCHEDULED_CASTE, preference=preference)
                break
